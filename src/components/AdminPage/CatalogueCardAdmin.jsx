  ...
  return (
    <div className="w-64 shadow-xl rounded-lg flex flex-col items-center">
      ...
      <div className="w-[90%] my-4 flex flex-col gap-2">
        <Link href={`/shop/detail-product/${data?.id}`} className="text-xl font-semibold">
          {data?.name}
        </Link>
        <div className="flex justify-between items-center">
          <p>Rp {data?.price}</p>
          <div className="flex h-[2rem]">
            <button className="w-20 border-[1px] rounded-full flex justify-center items-center bg-[#DE5959] text-white" onClick={() => router.push("/shop")}>
